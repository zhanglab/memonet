...
dir_overall <- paste0("x_vs_", ref1, "and", ref2)
...
for(subcluster in IDs){
  ...
  clusterY1 <- ref1
  print(paste0("clusterY1: ",clusterY1))
  clusterY2 <- ref2
  print(paste0("clusterY2: ",clusterY2))
  
  # subset to the pair of clusters
  clusters <- subset(clustersi, cluster_label %in% c(clusterX,clusterY1,clusterY2))
  ...
  metadata <- clusters %>% mutate(comparison = case_when(cluster_label == clusterX ~ "clusterX",
                                                         cluster_label == clusterY1 ~ "clusterY",
                                                         cluster_label == clusterY2 ~ "clusterY"))
  ...
  write.csv(res_tbl,
            paste0(dir, "/", clusterX, "_vs_", clusterY1 ,"and", clusterY2, "_all_genes.csv"),
            quote = FALSE,
            row.names = FALSE)
  ...
  write.csv(sig_res,
            paste0(dir, "/", clusterX, "_vs_", clusterY1,"and", clusterY2, "_sig_genes.csv"),
            quote = FALSE,
            row.names = FALSE)
}
