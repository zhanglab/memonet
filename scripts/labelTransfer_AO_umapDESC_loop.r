...
#loops through multiple clustering resutls
...
#umap_file <- sub("sil_scores_","umap",filename)

#----- begin loop -----#
filenames <- Sys.glob("/data/zhanglab/kdunton/6samples_cluster/deepseq_3_clustering/RNAseq/cluster_by_genes/DESC_parameter_test/test_loop/clusters*.csv")
for(file in filenames){
  filename <- basename(file)
  #this extracts the actual filename from the path, so you can extract the parameter details
  print(filename)
  # get the corresponding umap file
  umap_file <- sub("clusters","umap",filename)
  print(umap_file)
  # get outfile name
  outfile <- sub("clusters_dec22.","",filename)
  ...
  ## subset counts 
  print("dim:")
  print(dim(our_data.data))
  our_data_sub <- our_data.data[,metadata$X]
  print("dim:")
  print(dim(our_data_sub)) 

  ## make seurat obj
  our_data = CreateSeuratObject(counts = our_data_sub, min.cells = 3, project = "our_data")
  ...
  data.query <- sn_10X_AIBS
    #don't remove sn_10X_AIBS since it's outside loop
  ...
  ## load DESC umap data
  umap <- read.csv(paste0("/data/zhanglab/kdunton/6samples_cluster/deepseq_3_clustering/RNAseq/cluster_by_genes/DESC_parameter_test/test_loop/unrun_scenarios/",umap_file))
  ...
  write.csv(table, paste0(outfile,"_prediction_scores.csv"))
